...
public class DevelopmentCenterImpDAO extends GenericImplDao<DevelopmentCenter> implements DevelopmentCenterDAO{
	...
	public List<DevelopmentCenter> AvanticaDevelopmentCenter()
		...
		List<DevelopmentCenter> centers = (List<DevelopmentCenter>) sessionFactory.getCurrentSession().createQuery("SELECT dv.name FROM DevelopmentCenter dv WHERE dv.company = 'AVANTICA'").list();
		return centers;
	}
}
